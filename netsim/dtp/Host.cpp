...
Host::sync()
{
	...
	strcat(name,"_dest1");
        (((Host*)nd)->out_file).open (name, ios::out| ios::app);
...
{
      	//cout<<"aaaaaa"<<endl;
      	ofstream file1("testtt.txt", ios::out| ios::app);
      	DTPPacket *pkt22=new DTPPacket;
        copy_pkt(pkt22,pkt);
        Window1_iter head = recv_window.find(pkt22->id);
	if((head!=recv_window.end())&&!recv_window.empty())
	{
	        //handle_timer((void*)1);
	}
	else
	{
	        if((pkt22->id)==recv_so_far)
	        { 
	        //
	        if(wrote_so_far==recv_so_far&&recv_so_far)
                {
                        goto end1;      
	        }
	        if (out_file.is_open())
	        {
	        cout<<"aa:"<<pkt22->data<<endl;
	        out_file<<pkt22->data;
	        file1<<pkt22->data;
	        }
	        DTPPacket *pkt1;
	        Window1_iter head1 =recv_window.begin();
	       /* while(head1!=recv_window.end()&&!recv_window.empty())
	        {
	                pkt1=(*head1).second;
	         ...
	         wrote_so_far=  recv_so_far;     
                }
                else if((pkt22->id)>recv_so_far)
                {      
                        
                        cout<<"gggg"<<endl;
                        Window1Pair entry(pkt22->id,pkt22);
	                recv_window.insert(entry);
	        //set_timer(retrans, NULL);
	        }  
	 }          
	 //display(recv_window);
	end1: handle_timer((void*)1);
}
